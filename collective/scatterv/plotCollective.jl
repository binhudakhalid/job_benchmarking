...
function plot_bench(name::String; xlims=(1, 2 ^ 23), ylims=(Inf, Inf))
    ...
    julia  = readdlm(joinpath(@__DIR__, "j_scatterv.csv"), ',', Float64; skipstart=1)
    julia2 = readdlm(joinpath(@__DIR__, "scatter.csv"), ',', Float64; skipstart=1)


    p = plot(;
             title = "Latency of MPI Scatterv @Nocuta 2 (4 nodes, 256 ranks)",
             titlefont=font(10),
             xlabel = "message size",
             xscale = :log10,
             xlims,
