...
function plot_bench(name::String; xlims=(1, 2 ^ 23), ylims=(Inf, Inf))
    ...
    julia  = readdlm(joinpath(@__DIR__, "j.csv"), ',', Float64; skipstart=1)
    julia2 = readdlm(joinpath(@__DIR__, "osu.csv"), ',', Float64; skipstart=1)


    p = plot(;
             title = "Latency of MPI Scatter @Nocuta 2 (4 nodes, 256 ranks)",
             titlefont=font(10),
             xlabel = "message size",
             xscale = :log10,
             xlims,
    ...
    #plot!(p, riken[:, 1], riken[:, 2]; label="Cache avoidance (Riken-CCS)", marker=:auto, markersize=3)
    savefig(joinpath(@__DIR__, "scatter_1.pdf"))
