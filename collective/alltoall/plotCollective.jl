...
function plot_bench(name::String; xlims=(1, 2 ^ 23), ylims=(Inf, Inf))
    ...
    #julia  = readdlm(joinpath(@__DIR__, "j_bcast.csv"), ',', Float64; skipstart=1)
    julia = readdlm(joinpath(@__DIR__, "bcast.csv"), ',', Float64; skipstart=1)
    julia2 = readdlm(joinpath(@__DIR__, "bcast.csv"), ',', Float64; skipstart=1)

    p = plot(;
             title = "Latency of MPI Bcast @Nocuta 2 (4 nodes, 512 ranks)",
             titlefont=font(10),
             xlabel = "message size",
             xscale = :log10,
             xlims,
    ...
    #plot!(p, riken[:, 1], riken[:, 2]; label="Cache avoidance (Riken-CCS)", marker=:auto, markersize=3)
    savefig(joinpath(@__DIR__, "bcast_1.pdf"))

end

#plot_bench("Allreduce"; xlims=(4, 2 ^ 22.5), ylims=(10 ^ -6, Inf))
#plot_bench("Gatherv"; xlims=(1, 2 ^ 20.5))
plot_bench("ALLReduce"; xlims=(1, 2 ^ 23), ylims=(Inf, Inf))