...
function plot_bench(name::String; xlims=(1, 2 ^ 23), ylims=(Inf, Inf))
    ...
    julia  = readdlm(joinpath(@__DIR__, "julia_put_lock.csv"), ',', Float64; skipstart=1)
    julia2 = readdlm(joinpath(@__DIR__, "osu_put_lock.csv"), ',', Float64; skipstart=1)
    ...
    #plot!(p, riken[:, 1], riken[:, 2]; label="Cache avoidance (Riken-CCS)", marker=:auto, markersize=3)
    savefig(joinpath(@__DIR__, "$(lowercase(name))-latency_get_lock.pdf"))
