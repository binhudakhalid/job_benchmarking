...
function plot_bench(name::String; xlims=(1, 2 ^ 23), ylims=(Inf, Inf))
    ...
    julia  = readdlm(joinpath(@__DIR__, "j_put_fence.csv"), ',', Float64; skipstart=1)
    julia2 = readdlm(joinpath(@__DIR__, "put_fence.csv"), ',', Float64; skipstart=1)

    #title = "Latency of MPI Put with fence  @ Nocuta (4 nodes, 512 ranks)",

    p = plot(;
             title = "Latency of MPI Put with fence  @ Nocuta (2 nodes, 1 rank each)",
             titlefont=font(10),
             xlabel = "message size",
             xscale = :log10,
             xlims,
    ...
    #plot!(p, riken[:, 1], riken[:, 2]; label="Cache avoidance (Riken-CCS)", marker=:auto, markersize=3)
    savefig(joinpath(@__DIR__, "put_fence.pdf"))
